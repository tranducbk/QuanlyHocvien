...
const getWeeklySlot = (weekly, day) => weekly?.[day] || weekly?.[day.toLowerCase()] || {};

const summarizeWeeklySchedules = (records) => {
  const daily = Object.fromEntries(
    days.map(day => [day, { morning: 0, noon: 0, evening: 0 }])
  );

  for (const record of records) {
    const weekly = record.weekly || record.get?.('weekly') || {};
    for (const day of days) {
      const daySlot = getWeeklySlot(weekly, day);
      for (const slot of slots) {
        daily[day][slot.key] += Number(Boolean(daySlot[slot.key]));
      }
    }
  }

  return { totalSchedules: records.length, daily };
};
...
const getAll = async (query, requester) => {
  ...
  const summaryWhere = { ...optionsWhere };
  for (const field of ['userId', 'semesterId', 'isAutoGenerated']) {
    if (query[field] !== undefined) summaryWhere[field] = query[field];
  }

  const summaryInclude = cloneInclude([include[0]]);
  summaryInclude[0].attributes = [];
  summaryInclude[0].include[0].attributes = [];
  const summaryRows = await CutRice.findAll({
    attributes: ['weekly'],
    where: summaryWhere,
    include: summaryInclude,
  });

  result.summary = summarizeWeeklySchedules(summaryRows);
